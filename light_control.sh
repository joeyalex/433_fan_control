...
echo `whoami` >> /fan_control/log.txt
...
if [ "$1" = "Set" ]; then
   ...
   if [ "$3" = "On" ]; then
      ...
      if [ "$4" = "true" ]; then
         ...
         /fan_control/fan_send2 LIGHT_ON $remote #>> /dev/null 2>&1
         ...
         /fan_control/fan_send2 LIGHT_OFF $remote #>> /dev/null 2>&1
